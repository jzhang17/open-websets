...
# React Data GridCell Text Selection

![react logo](/_astro/react.CtDRhtxt.svg)React

The grid can be configured to allow simple text selection.

![Cell Text Selection](/_astro/cellTextSelection.Cah3lrQ8.png) 

##  Enabling Cell Text Selection [Copy Link](#enabling-cell-text-selection) 
...
When `enableCellTextSelection` is set to `true`, the default [Clipboard](/react-data-grid/clipboard/) behaviour is disabled, and only the selected text is copied.

* [Cell Text Selection](#top)
* [Enabling Cell Text Selection](#enabling-cell-text-selection)
