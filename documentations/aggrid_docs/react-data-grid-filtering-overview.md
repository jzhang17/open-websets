...
## Filter Types [Copy Link](#filter-types)

* **[Column Filter](/react-data-grid/filtering/)** - a column-based filter that filters rows using the values under a specific column:

  ![Text Filter](/_astro/column-filters.DDUmHzvl.png)
* **[Quick Filter](/react-data-grid/filter-quick/)** - a text-based filter that filters rows across all columns and words in the text:

  ![Quick Filter](/_astro/quick-filter.ChPYWlOJ.png)
* **[External Filter](/react-data-grid/filter-external/)** - an application filter that is independent of any filtering done within the grid:

  ![External Filter](/_astro/external-filter.vSRKlPW8.png)
* **[Advanced Filter](/react-data-grid/filter-advanced/)** - a column-based filter that filters rows across all columns with type-ahead support and an advanced visual builder:

  ![Advanced Filter](/_astro/advanced-filter.C4pzLplo.png)

## Next Up [Copy Link](#next-up)

Continue to the next section to learn about [Column Filters](/react-data-grid/filtering/).

* [Filtering Overview](#top)
