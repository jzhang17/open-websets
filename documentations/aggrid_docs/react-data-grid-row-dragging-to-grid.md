...
# React Data GridRow Dragging Between Grids
...
| getRowDropZoneParams[Copy Link](#reference-rowDrag-getRowDropZoneParams)  Function  Returns the `RowDropZoneParams` to be used by another grid's `addRowDropZone` method.  [RowDragModule](/react-data-grid/modules/) |

## Adding a Grid as Target [Copy Link](#adding-a-grid-as-target)
...
## Dragging Multiple Records Between Grids [Copy Link](#dragging-multiple-records-between-grids)

It is possible to drag multiple records at once from one grid to another.

In the example below, note the following:

* This example allows for `rowDragMultiRow`, between grids. For more info on `multiRowDrag` within the grid see the [Multi-Row Dragging](/react-data-grid/row-dragging/#multi-row-dragging) section in the Row Dragging documentation.
...
* [Row Dragging Between Grids](#top)
* [Adding a Grid as Target](#adding-a-grid-as-target)
* [Dragging Multiple Records Between Grids](#dragging-multiple-records-between-grids)
