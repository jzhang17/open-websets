...
# React Data GridLegacy Themes: Customising Selections

![react logo](/_astro/react.CtDRhtxt.svg)React

Control how selected rows and cells appear.

This page describes the grid's legacy theming system that was the default in v32 and before, for the benefit of applications that have not yet migrated to the Theming API. These themes are deprecated and will be removed in a future major version. You may want to visit the [new theming docs](/react-data-grid/theming-selections/) or check out the [migration guide](/react-data-grid/theming-migration/).

##  Row Selections [Copy Link](#row-selections) 

When [row selection](/react-data-grid/row-selection/) is enabled, you can set the color of selected rows using `--ag-selected-row-background-color`. If your grid uses alternating row colours we recommend setting this to a semi-transparent colour so that the alternating row colours are visible below it.
...
###  Cell Selection CSS Variables [Copy Link](#cell-selection-css-variables) 
...
* [Legacy Themes: Customising Selections](#top)
* [Row Selections](#row-selections)
* [Cell Selections](#cell-selections)
* [Cell Selection CSS Variables](#cell-selection-css-variables)
