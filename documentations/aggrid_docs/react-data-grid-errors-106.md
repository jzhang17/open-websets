...
Theming API and Legacy Themes are both used in the same page. A Theming API theme has been provided to the 'theme' grid option, but the file (ag-grid.css) is also included and will cause styling issues. Remove ag-grid.css from the page. See the migration guide: https://www.ag-grid.com/javascript-data-grid/theming-migration/

To show this error text in the dev console, import the `ValidationModule`.

##  Mixed Theming Approaches [Copy Link](#mixed-theming-approaches) 

In v33 the default approach to theming AG Grid has been updated to use the [Theming API](/react-data-grid/theming/). This error occurs when a theme is set via the `theme` property but the legacy CSS files have not been removed.

For a comprehensive guide on migrating from v32 please see [Migrating to the Theming API](/react-data-grid/theming-migration/).
...
###  Adopt the new Theming API [Copy Link](#adopt-the-new-theming-api) 

To adopt the new Theming API please complete the steps outlined in [Adopt the Theming API](/react-data-grid/theming-migration/#adopt-the-theming-api).

###  Continue with legacy Themes [Copy Link](#continue-with-legacy-themes) 

Set the grid option: `theme='legacy'`. See [Continue with Legacy Themes](/react-data-grid/theming-migration/#continue-with-legacy-themes) for more information.
...
* [AG Grid React Error #106](#top)
* [Full Error Text](#reference-full-error-text)
* [Mixed Theming Approaches](#mixed-theming-approaches)
* [Adopt the new Theming API](#adopt-the-new-theming-api)
