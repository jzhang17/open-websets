...
# React Data GridUpgrading to AG Grid 32.1

![react logo](/_astro/react.CtDRhtxt.svg)

## What's New [Copy Link](#whats-new)

See the [release post](https://blog.ag-grid.com/whats-new-in-ag-grid-32-1/) for details of what's new in version 32.1.

## Codemods [Copy Link](#codemods)

Follow these steps to upgrade your project's AG Grid version to `32.1.0`:

1. Locate your project's `package.json` and note the version of AG Grid that you are currently using.
2. Update any AG Grid dependencies listed in the `package.json` to version `32.1.0`.
3. Open a terminal and navigate to your project's root folder.
4. Run the `migrate` command of version `32.1` of the AG Grid codemod runner, where `$FROM_VERSION` refers to your project's existing AG Grid version:

   ```
   npx @ag-grid-devtools/cli@32.1 migrate --from=$FROM_VERSION
