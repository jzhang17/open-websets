...
# React Data GridText Cell Editor
...
## Enabling Text Cell Editor [Copy Link](#enabling-text-cell-editor)
...
|  |
| --- |
| useFormatter[Copy Link](#reference-ITextCellEditorParams-useFormatter)  boolean  If `true`, the editor will use the provided `colDef.valueFormatter` to format the value displayed in the editor. Used when the cell value needs formatting prior to editing, such as when using reference data and you want to display text rather than code. |
| maxLength[Copy Link](#reference-ITextCellEditorParams-maxLength)  number  default: 524288  Max number of characters to allow. |

## Next Up [Copy Link](#next-up)

Continue to the next section: [Large Text Cell Editor](/react-data-grid/provided-cell-editors-large-text/).

* [Text Cell Editor](#top)
* [Enabling Text Cell Editor](#enabling-text-cell-editor)
