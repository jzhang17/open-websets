...
[![React Floating Filters thumbnail](https://img.youtube.com/vi/MdO6vZN2Gxo/0.jpg)](https://www.youtube.com/watch?v=MdO6vZN2Gxo)

## Example: Custom Floating Filter [Copy Link](#example-custom-floating-filter)
...
## Implementing a Floating Filter Component [Copy Link](#implementing-a-floating-filter-component)
...
## Custom Floating Filter Parameters [Copy Link](#custom-floating-filter-parameters)

### Floating Filter Props [Copy Link](#floating-filter-props)
...
|  |
| --- |
| model[Copy Link](#reference-CustomFloatingFilterProps-model)  TModel | null  The current filter model for the component. |
| onModelChange[Copy Link](#reference-CustomFloatingFilterProps-onModelChange)  Function  Callback that should be called every time the model in the component changes. |
| column[Copy Link](#reference-CustomFloatingFilterProps-column)  [Column](/react-data-grid/column-object/)  The column this filter is for. |
| filterParams[Copy Link](#reference-CustomFloatingFilterProps-filterParams)  IFilterParams  The params object passed to the filter. This is to allow the floating filter access to the configuration of the parent filter. For example, the provided filters use debounceMs from the parent filter params. |
| currentParentModel[Copy Link](#reference-CustomFloatingFilterProps-currentParentModel)  Function  This is a shortcut to invoke getModel on the parent filter. If the parent filter doesn't exist (filters are lazily created as needed) then it returns null rather than calling getModel() on the parent filter. |
| parentFilterInstance[Copy Link](#reference-CustomFloatingFilterProps-parentFilterInstance)  Function  Gets a reference to the parent filter. The result is returned asynchronously via a callback as the parent filter may not exist yet. If it does not exist, it is created and asynchronously returned (AG Grid itself does not create components asynchronously, however if providing a framework provided filter e.g. React, it might be). The floating filter can then call any method it likes on the parent filter. The parent filter will typically provide its own method for the floating filter to call to set the filter. For example, if creating custom filter A, it should have a method your floating A can call to set the state when the user updates via the floating filter. |
| showParentFilter[Copy Link](#reference-CustomFloatingFilterProps-showParentFilter)  Function  Shows the parent filter popup. |
| api[Copy Link](#reference-CustomFloatingFilterProps-api)  [GridApi](/react-data-grid/grid-api/)  The grid api. |
| context[Copy Link](#reference-CustomFloatingFilterProps-context)  TContext  Application context as set on `gridOptions.context`. |

### Floating Filter Callbacks [Copy Link](#floating-filter-callbacks)
...
## Example: Custom Filter And Custom Floating Filter [Copy Link](#example-custom-filter-and-custom-floating-filter)
...
## Example: Custom Filter And Read-Only Floating Filter [Copy Link](#example-custom-filter-and-read-only-floating-filter)
...
## Sliding Floating Filters [Copy Link](#sliding-floating-filters)
...
Continue to the next section to learn about [Advanced Filters](/react-data-grid/filter-advanced/).

* [Floating Filter Component](#top)
* [Example: Custom Floating Filter](#example-custom-floating-filter)
* [Implementing a Floating Filter Component](#implementing-a-floating-filter-component)
* [Custom Floating Filter Parameters](#custom-floating-filter-parameters)
* [Floating Filter Props](#floating-filter-props)
* [Floating Filter Callbacks](#floating-filter-callbacks)
* [Floating Filter Lifecycle](#floating-filter-lifecycle)
* [Example: Custom Filter And Custom Floating Filter](#example-custom-filter-and-custom-floating-filter)
* [Example: Custom Filter And Read-Only Floating Filter](#example-custom-filter-and-read-only-floating-filter)
* [Sliding Floating Filters](#sliding-floating-filters)
* [Next Up](#next-up)
