...
* `--ag-list-item-height` sets the height of items in lists of values, such as the [rich select editor](/react-data-grid/provided-cell-editors-rich-select/) and [set filter](/react-data-grid/filter-set/).

In the following example, classes are applied to the grid container that change compactness dynamically:

## Full list of compactness variables [Copy Link](#full-list-of-compactness-variables)
...
| --ag-widget-horizontal-spacing[Copy Link](#reference-variables---ag-widget-horizontal-spacing)  CSS length (e.g. `0`, `4px` or `50%`)  The horizontal spacing between widgets in containers that contain horizontally stacked widgets such as the column groups header component. |
...
## Using CSS rules to customise compactness [Copy Link](#using-css-rules-to-customise-compactness)
...
To change the height of rows, headers and list item heights, you *must* use the provided CSS variables (`--ag-row-height`, `--ag-header-height` and `--ag-list-item-height`). The grid uses [DOM virtualisation](/react-data-grid/dom-virtualisation/) for rendering large amounts of data, and needs to know the height of virtualised elements in order to calculate their layout. It does this by measuring the values of CSS variables such as `--ag-row-height`.

Using CSS to set a height on a row element itself e.g. `.ag-row { height: 50px; }` is not supported.

* [Legacy Themes: Customising Compactness & Row Height](#top)
* [Full list of compactness variables](#full-list-of-compactness-variables)
* [Using CSS rules to customise compactness](#using-css-rules-to-customise-compactness)
* [Setting the height of rows, headers and list items](#setting-the-height-of-rows-headers-and-list-items)
