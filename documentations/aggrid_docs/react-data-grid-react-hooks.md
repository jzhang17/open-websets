...
###  Immutable Data [Copy Link](#immutable-data) 
...
See [Updating Row Data](/react-data-grid/data-update-row-data/) for more information and other benefits of providing the `getRowId` callback.

##  Column Definitions [Copy Link](#column-definitions) 
...
##  Object Properties [Copy Link](#object-properties) 
...
##  Simple Properties [Copy Link](#simple-properties) 
...
##  Callbacks [Copy Link](#callbacks) 

For [Grid Options](/react-data-grid/grid-options/) that accept functions, i.e `isRowSelectable` we strongly recommend you use `useCallback` to avoid resetting grid state on every render. For example, if you do not use a callback for `isRowSelectable` then on every render the grid will receive a new function and have to re-run selection logic.
...
##  Event Listeners [Copy Link](#event-listeners) 

For [Event Listeners](/react-data-grid/grid-events/) there is no requirement to use `useCallback` as event handlers do not trigger updates within the grid. However, you may find it easier to be consistent with Callbacks and just always use `useCallback`.
...
Custom Components can be referenced by Name or Direct Reference, see [Registering Components](/react-data-grid/components/). When providing a Direct Reference to the component AG Grid will avoid most unnecessary renders. However, if your component is rendered more than you expect, it may help to wrap it with `memo`.
...
##  Debug Mode [Copy Link](#debug-mode) 
