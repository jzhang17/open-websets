...
## Content Security Policy (CSP) [Copy Link](#content-security-policy-csp)
...
### script-src [Copy Link](#script-src)
...
1. Many grid properties such as [Cell Class Rules](/react-data-grid/cell-styles/#cell-class-rules) and [Value Getters](/react-data-grid/value-getters/) allow using expressions. However, this requires `script-src 'unsafe-eval'`. In order to avoid this requirement, replace expressions with functions as shown in the example below for the cellClassRules property of a column definition:
...
2. By default, [Advanced Filter](/react-data-grid/filter-advanced/) also requires `script-src 'unsafe-eval'`, but this can be avoided by [Suppressing Advanced Filter Function Evaluation](/react-data-grid/filter-advanced/#suppress-advanced-filter-function-evaluation).

### style-src [Copy Link](#style-src)

[Themes](/react-data-grid/theming/) work by injecting CSS styles into the DOM when the grid initialises. By default this requires a `style-src 'unsafe-inline'` directive in your CSP.
...
CSP nonces are global to a page, so when a page has multiple grids, all grids must have the same styleNonce set. Consider using [global grid options](/react-data-grid/grid-interface/#global-grid-options) to set the styleNonce property to all grids.

### img-src [Copy Link](#img-src)

The `img-src data:` directive is required because the grid uses data urls to embed SVG images in CSS files.

### font-src [Copy Link](#font-src)

If you are using [legacy themes](/react-data-grid/theming-v32/) then the `font-src data:` directive is required. This is because legacy themes use data urls to embed the icon font CSS files. Theming API themes use SVG icons and so do not require this.
...
### Example locked-down CSP [Copy Link](#example-locked-down-csp)
...
3. You are using [Theming API](/react-data-grid/theming/) instead of [legacy themes](/react-data-grid/theming-v32/), and are not using loadThemeGoogleFonts
4. The random string passed to `gridOptions.styleNonce` on this page load is "123xyz"

## Security Vulnerability Testing [Copy Link](#security-vulnerability-testing)
...
## SonarQube Results [Copy Link](#sonarqube-results)
...
* [img-src](#img-src)
* [font-src](#font-src)
* [Example locked-down CSP](#example-locked-down-csp)
* [Security Vulnerability Testing](#security-vulnerability-testing)
* [SonarQube Results](#sonarqube-results)
