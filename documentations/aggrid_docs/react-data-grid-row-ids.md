...
## Row Data [Copy Link](#row-data)

When using the default row model - [Client Side](/react-data-grid/row-models/#client-side) data is provided to the grid via the `rowData` property.
...
If you are using TypeScript you may wish to provide the grid with your row data type for an improved developer experience. See [TypeScript Generics](/react-data-grid/typescript-generics/) for more details.

## Updating Row Data [Copy Link](#updating-row-data)

The simplest way to update `rowData` is to pass a new array of data to the grid. For full details on updating row data, including transactions, see [Updating Data](/react-data-grid/data-update/).

## Row IDs [Copy Link](#row-ids)
...
|  |
| --- |
| getRowId[Copy Link](#reference-rowModels-getRowId)  GetRowIdFunc  [Initial](/react-data-grid/grid-interface/#initial-grid-options)  Provide a pure function that returns a string ID to uniquely identify a given row. This enables the grid to work optimally with data changes and updates. |

## Row Nodes [Copy Link](#row-nodes)

Every row displayed in the grid is represented by a [Row Node](/react-data-grid/row-interface/) which exposes stateful attributes and methods for directly interacting with the row.

Row Nodes are accessed via [Grid API](/react-data-grid/grid-api/) methods, as well as provided as props for items such as [Cell Component](/react-data-grid/component-cell-renderer/).

The following buttons log the data to the developer console.

Check the [Row Reference](/react-data-grid/row-object/) and [Row Events](/react-data-grid/row-events/) for all items available on the [Row Node](/react-data-grid/row-interface/).

* [Row Data](#top)
* [Row Data](#row-data)
* [Updating Row Data](#updating-row-data)
* [Row IDs](#row-ids)
* [Row Nodes](#row-nodes)
