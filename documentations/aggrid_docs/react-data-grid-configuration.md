...
## Defining Columns [Copy Link](#defining-columns)

Each column in the grid is defined using a [Column Definition](/react-data-grid/column-definitions/), which is a JavaScript key-value object consisting of [Column Options](/react-data-grid/column-properties/). An array of these objects can be passed to the `columnDefs` [Grid Option](/react-data-grid/grid-options/#reference-columns-columnDefs) and the grid will create matching columns.
...
Columns can be also be configured under [Column Groups](/react-data-grid/column-groups/), which present the columns under shared headers. These can be configured by adding a level of nesting to the column definition.
...
## Referencing Columns [Copy Link](#referencing-columns)
...
### Column IDs [Copy Link](#column-ids)
...
### Column Objects [Copy Link](#column-objects)

Every column displayed in the grid is represented by a [Column Object](/react-data-grid/column-interface/#column) which has attributes, methods and events for interacting with the specific column e.g. `column.isVisible()`.

Columns can be accessed via Grid API methods, and provided as parameters from some [Grid Events](/react-data-grid/grid-events/#reference-columns).

The [Column Reference](/react-data-grid/column-object/) displays a list of functions available on the Column Object.

It is also possible to listen for [Column Events](/react-data-grid/column-events/) by attaching an [Event Listener](/react-data-grid/column-object/#reference-events-addEventListener).
...
[Column State](/react-data-grid/column-state/) should be used when restoring a users grid, for example saving and restoring column widths.

[Update Column Definitions](/react-data-grid/column-updating-definitions/#changing-column-definition) to modify properties that the user cannot control, and as such are not supported by Column State. Whilst column definitions can be used to change stateful properties, this can cause additional side effects.
